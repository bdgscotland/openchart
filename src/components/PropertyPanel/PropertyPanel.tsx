...
import { ChevronDown, ChevronRight, Settings, Type, Palette, Layout, Layers, Link } from 'lucide-react';
...
const AdvancedColorPicker: React.FC<AdvancedColorPickerProps> = ({
  ...
  const handleSwatchClick = () => {
    console.log('🎨 AdvancedColorPicker swatch clicked:', { label, isPickerOpen });
    setIsPickerOpen(true);
  };

  const handleColorChange = useCallback((color: string) => {
    console.log('🎨 AdvancedColorPicker handleColorChange called:', { label, color });
    setTempValue(color);
    // Call onChange immediately without debouncing to reduce flashing
    onChange(color);
  }, [onChange, label]);

  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const color = e.target.value;
    console.log('🎨 AdvancedColorPicker input change:', { label, color });
    setTempValue(color);
...
const NumberInput: React.FC<NumberInputProps> = ({
  ...
  const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('🎨 NumberInput handleChange called:', { label, inputValue });
    setTempValue(inputValue);

    const numValue = parseFloat(inputValue) || 0;
    const clampedValue = Math.max(min || -Infinity, Math.min(max || Infinity, numValue));

    // Call onChange immediately without debouncing
    onChange(clampedValue);
  }, [onChange, min, max, label]);
...
export const PropertyPanel: React.FC<PropertyPanelProps> = ({
  ...
  const [expandedSections, setExpandedSections] = useState({
    ...
    position: false,
    size: false,
    bulk: selectedElements.length > 1,
    sync: false,
  });
  ...
  const handleStyleUpdate = useCallback((styleUpdate: Partial<ElementStyle>) => {
    console.log('🎨 PropertyPanel handleStyleUpdate called:', { styleUpdate, selectedElementsCount: selectedElements.length });

    // Apply updates immediately to all selected elements
    selectedElements.forEach(element => {
      console.log('🎨 PropertyPanel calling onUpdateElementStyle:', { elementId: element.id, styleUpdate });
  ...
  const handleTextUpdate = useCallback((text: string) => {
    ...
    console.log('🎨 PropertyPanel handleTextUpdate called:', { elementId: primaryElement.id, text });
  ...
  const handleAspectRatioToggle = useCallback((locked: boolean) => {
    // Could be stored in element properties or component state
  ...
  return (
    <div className={`property-panel property-panel-expanded ${
      isLoading ? 'loading' : ''
    } ${className}`}>
              ...
              >
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      console.log('🎨 Text input onChange triggered:', { value: e.target.value, primaryElementId: primaryElement?.id });
              ...
              <div className={`property-section-wrapper ${
                ...
                <div className="form-row">
                  <AdvancedColorPicker
                    ...
                    onChange={(value) => {
                      console.log('🎨 Fill Color onChange triggered:', { value, primaryElementId: primaryElement?.id });
                      handleStyleUpdate({ fill: value });
                    }}
                  />
                  <AdvancedColorPicker
                    label="Border Color"
                    ...
                    onChange={(value) => {
                      console.log('🎨 Border Color onChange triggered:', { value, primaryElementId: primaryElement?.id });
                ...
                <div className="form-row">
                  <NumberInput
                    ...
                    onChange={(value) => {
                      console.log('🎨 Border Width onChange triggered:', { value, primaryElementId: primaryElement?.id });
                  ...
                  <NumberInput
                    ...
                    onChange={(value) => {
                      console.log('🎨 Opacity onChange triggered:', { value, primaryElementId: primaryElement?.id });
